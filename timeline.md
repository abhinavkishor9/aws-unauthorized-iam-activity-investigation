...
This timeline records the major activities, observations, and decisions made during the AWS IAM and CloudTrail investigation.
...
The overall investigation flow was:

1. AWS Academy environment accessed.
2. IAM Dashboard reviewed.
3. IAM users reviewed.
4. IAM roles reviewed.
5. IAM policies reviewed.
6. Highly privileged policies identified.
7. CloudTrail Event History opened.
8. CloudTrail lookup attempted.
9. `AccessDeniedException` observed.
10. `cloudtrail:LookupEvents` identified as the denied action.
11. Explicit Service Control Policy denial identified.
12. CloudTrail visibility limitation documented.
13. IAM and CloudTrail findings correlated.
14. Potential IAM abuse events identified for future hunting.
15. Evidence assessed.
16. Investigation concluded without confirming compromise.

---

## Final Timeline Conclusion

The investigation established that the AWS environment contained no permanent IAM users, 26 IAM roles, and a broad IAM policy library containing highly privileged policies such as `AdministratorAccess`. The primary investigative limitation occurred when CloudTrail Event History could not be queried because the active AWS role was explicitly denied `cloudtrail:LookupEvents` by a Service Control Policy. Therefore, the investigation did not confirm unauthorized IAM activity, but it also could not definitively rule it out due to restricted CloudTrail visibility.
